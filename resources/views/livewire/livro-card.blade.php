<li class="splide__slide list-none" wire:ignore.self>
    <div class="bg-white rounded-lg shadow-lg overflow-hidden group h-full flex flex-col {{ $size === 'large' ? 'max-w-xs' : '' }}">
        ...
        <div class="flex-shrink-0 relative">
            ...
            <a href="javascript:void(0)" wire:click="openModal({{ $livro['id'] }})" class="block cursor-pointer">
                @if($urlCapa)
                    <img src="{{ $urlCapa }}"
                         alt="Capa do livro {{ $livro['titulo'] ?? $livro->titulo }}"
                         class="w-full object-cover aspect-[2/3] group-hover:scale-105 transition-transform duration-300">
                @else
                    <div class="w-full bg-biblioteca-100 aspect-[2/3] flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
